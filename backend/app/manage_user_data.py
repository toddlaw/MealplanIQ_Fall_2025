...
def process_user_data(db, user_id, extract_data):
    ...
    db.update_user_favourite_cuisines(user_id, extract_data['favourite_cuisines'])
    db.insert_or_update_user_religious_constraint(user_id, extract_data['religious_constraint'])
...
def create_data_input_for_auto_gen_meal_plan(db, user_id):
    ...
    dietary_constraint = db.retrieve_user_dieatary_constraints(user_id)[0].lower()
    health_goal = db.retrieve_user_health_goal(user_id)
    religious_constraint = db.retrieve_user_religious_constraints(user_id)[0].lower()
...
def _get_min_and_max_date_from_the_last_date(db, user_id):
    last_meal_plan_date_in_milliseconds = db.retrieve_user_last_date_plan_profile(user_id)
    timestamp_seconds = last_meal_plan_date_in_milliseconds / 1000.0
    base_date = datetime.datetime.fromtimestamp(timestamp_seconds, datetime.timezone.utc)
